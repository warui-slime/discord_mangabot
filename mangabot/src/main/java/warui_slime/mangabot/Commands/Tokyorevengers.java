...
public class Tokyorevengers extends ListenerAdapter
{
    public static int pg=1;
    public static int ch;
    public static int num_pages;
    public static int max_ch = 273;

    public String[] Trchap(int ch) throws Exception
    {
        
        Document doc = Jsoup.connect("https://www.read-tokyorevengers.com/manga/tokyo-revengers-chapter-"+ch+"/").get();
    ...
    public void trpagemaker(MessageChannelUnion chan,String[] pages)
    {
        EmbedBuilder send_ch = new EmbedBuilder();
        send_ch.setColor(Color.green);
        send_ch.setTitle("Tokyo Revengers Chapter "+ch);
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        if(args[0].equalsIgnoreCase(Mangabot.prefix+"tr") && args.length>1)
        {
            if(Integer.parseInt(args[1])<=max_ch)
            {
                ...
                try
                {
                    ...
                    Mangabot.curr_manga = "TR";
                    Mangabot.msguser = event.getAuthor();
                    trpagemaker(event.getChannel(),Trchap(ch));
                }
                catch (Exception e)
                {
